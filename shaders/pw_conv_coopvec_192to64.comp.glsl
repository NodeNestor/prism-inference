...
// Pointwise conv 192->64 using cooperative vectors (tensor cores)
// CHW format input/output. Each thread = one pixel.
// Used in dec2: upsample(128ch) + skip(64ch) = 192ch -> 64ch
...
layout(push_constant) uniform PC {
    ...
    int width;
    int height;
    int weight_offset;  // [64 x 192] row-major
    int bias_offset;    // [64]
...
void main() {
    ...
    // Gather 192 channels for this pixel from CHW layout
    coopvecNV<float16_t, 192> inp;
    for (int c = 0; c < 192; c++) {
        inp[c] = features[input_offset + c * pixels + idx];
    }

    // Tensor core matmul: [64 x 192] x [192] -> [64]
