...
// MoE FFN: 2 experts, 256→256→256 each. Compile-time unrolled.
...
void main() {
    ...
    coopVecMatMulAddNV(logits, inp, gl_ComponentTypeFloat16NV,
        ...
        2, 256, gl_CooperativeVectorMatrixLayoutRowMajorNV, false, 256*2);
    int best = 0; float bv = float(logits[0]);
    for (int e = 1; e < 2; e++) { float v = float(logits[e]); if (v > bv) { bv = v; best = e; } }
