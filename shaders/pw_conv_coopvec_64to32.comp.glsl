...
// Pointwise conv 64->32 using cooperative vectors (tensor cores)
...
layout(push_constant) uniform PC {
    ...
    int width;
    int height;
    int weight_offset;  // [32 x 64] row-major
    int bias_offset;    // [32]
...
void main() {
    ...
    // Gather 64 channels for this pixel from CHW layout
    coopvecNV<float16_t, 64> inp;
    for (int c = 0; c < 64; c++) {
        inp[c] = features[input_offset + c * pixels + idx];
    }

    // Tensor core matmul: [32 x 64] x [64] -> [32]
